...
function syncResponsiveConfigWithCSS() {
  ...
  RESPONSIVE_CONFIG.breakpoints.xs = parsePx(
    "--bp-xs",
    RESPONSIVE_CONFIG.breakpoints.xs,
  );
  RESPONSIVE_CONFIG.breakpoints.sm = parsePx(
    "--bp-sm",
    RESPONSIVE_CONFIG.breakpoints.sm,
  );
  RESPONSIVE_CONFIG.breakpoints.md = parsePx(
    "--bp-md",
    RESPONSIVE_CONFIG.breakpoints.md,
  );
  RESPONSIVE_CONFIG.breakpoints.lg = parsePx(
    "--bp-lg",
    RESPONSIVE_CONFIG.breakpoints.lg,
  );
  RESPONSIVE_CONFIG.breakpoints.xl = parsePx(
    "--bp-xl",
    RESPONSIVE_CONFIG.breakpoints.xl,
  );
  RESPONSIVE_CONFIG.breakpoints.xxl = parsePx(
    "--bp-2xl",
    RESPONSIVE_CONFIG.breakpoints.xxl,
  );
}
...
function getEffectiveDimensions(containerWidth = 700, containerHeight = 450) {
  ...
  const width = Math.max(
    280,
    Math.min(containerWidth, viewportWidth - horizontalChrome),
  );
  const viewportLimitedHeight = Math.max(260, viewportHeight - verticalChrome);
  const height = Math.max(
    config.minHeight,
    Math.min(containerHeight, viewportLimitedHeight),
  );
  ...
  window.addEventListener("resize", () => {
    ...
    graphResizeTimeout = setTimeout(() => {
      ...
      const heightDiff = Math.abs(
        window.innerHeight - lastGraphWindowSize.height,
      );
...
const glossaryTerms = [
  {
    ...
    definition:
      "Парадигма программирования, которая описывает процесс вычисления в виде последовательности команд, изменяющих состояние программы.",
    relation:
      "В нашем коде: Используется для управления состоянием игровых моделей через последовательные операции.",
  ...
  },
  {
    ...
    definition:
      "Логическая операция, возвращающая истину, если ровно один из операндов истинен. Обозначается символом XOR или ^.",
    relation:
      "В матричной модели: Каждое нажатие инвертирует ячейку и соседей через XOR.",
...
<span class="comment">// 0 -> 1, 1 -> 0</span>`,
  },
  {
    ...
    definition:
      "Алгоритм поиска кратчайшего пути от одной вершины графа до всех остальных. Работает только с неотрицательными весами ребер.",
    relation:
      "В графовой модели: Используется для нахождения оптимального маршрута с учетом ограничений.",
  ...
  },
  {
    ...
    definition:
      'Мера хаоса или неупорядоченности в системе. В контексте алгоритмов - количество "беспорядка", которое необходимо устранить.',
    relation:
      "Отображается как метрика: Уменьшается при правильных решениях пользователя.",
  ...
  },
  {
    ...
    definition:
      "Группа, порожденная одним элементом. Все элементы получаются последовательным применением операции к порождающему элементу.",
    relation:
      "В модели колец: Вращение колец образует циклическую группу состояний.",
...
<span class="comment">// Всегда в диапазоне [0, max)</span>`,
  },
  {
    ...
    definition:
      "Граф, в котором ребра имеют направление. Ребро (u, v) позволяет перейти только от u к v, но не наоборот.",
    relation:
      "В Kaspi-модели: Транзакции имеют направление от отправителя к получателю.",
  ...
  },
  {
    ...
    definition:
      "Способ представления графа в виде квадратной матрицы, где элемент a[i][j] указывает на наличие и вес ребра между вершинами i и j.",
...
<span class="keyword">const</span> adj = [
  ...
  },
  {
    title: "Детерминизм",
    definition:
      "Свойство системы, при котором одинаковые входные данные всегда приводят к одинаковому результату. Отсутствие случайности.",
    relation:
      "Все модели детерминированы: Одни и те же действия дают предсказуемый результат.",
...
<span class="keyword">function</span> <span class="function">nextState</span>(current, action) {
  ...
  },
  {
    title: "Декомпозиция",
    definition:
      "Процесс разделения сложной системы или задачи на более мелкие, простые и управляемые части (подсистемы), которые легче анализировать и реализовывать.",
    relation:
      "Позволяет справиться со сложностью: решение большой задачи сводится к последовательному решению набора малых задач.",
...
<span class="keyword">function</span> <span class="function">processUser</span>(user) {
  ...
  },
  {
    title: "Формализация",
    definition:
      "Переход от содержательного описания объекта или процесса на естественном языке к строгому описанию с помощью искусственного языка (математики, логики или кода).",
    relation:
      "Устраняет двусмысленность: превращает абстрактную идею в четкий алгоритм, понятный машине.",
...
<span class="keyword">const</span> <span class="variable">isAllowed</span> = user.age >= <span class="number">18</span>;`,
  },
  {
    ...
    definition:
      "Соответствие между двумя структурами, при котором сохраняются все связи и свойства. Если структуры изоморфны, операции в одной полностью соответствуют операциям в другой.",
    relation:
      "В разработке: код, который может выполняться и на сервере, и на клиенте с сохранением идентичной логики и состояния.",
...
<span class="keyword">function</span> <span class="function">renderApp</span>(state) {
  ...
  },
  {
    ...
    definition:
      "Стиль программирования, при котором описывается *что* программа должна достичь (желаемый результат), а не пошаговое руководство *как* это сделать.",
    relation:
      "Снижает количество побочных эффектов и делает код более читаемым за счет абстрагирования от управления состоянием и циклами.",
...
<span class="comment">// Мы не пишем цикл 'for', мы описываем КРИТЕРИЙ выбора</span>`,
  },
  {
    ...
    definition:
      "Сервис для оценки транзакций или действий на предмет вероятности мошенничества (фрода) в режиме реального времени с использованием правил и ML-моделей.",
    relation:
      "Защищает бизнес от потерь: анализирует паттерны поведения, геолокацию и отпечатки устройств для блокировки подозрительных операций.",
...
<span class="keyword">const</span> <span class="function">isFraud</span> = (tx) => tx.amount > <span class="number">10000</span> && !tx.isVerifiedDevice;`,
  },
  {
    title: "Аппроксимация",
    definition:
      "Метод замены сложных математических объектов (функций, значений) более простыми и близкими к оригиналу, но удобными для вычислений.",
    relation:
      "Используется, когда точное решение найти невозможно или слишком дорого по ресурсам: мы жертвуем точностью ради скорости.",
...
<span class="keyword">const</span> <span class="variable">PI_APPROX</span> = <span class="number">3.14</span>; <span class="comment">// Для многих задач этой точности достаточно</span>`,
  },
  {
    ...
    definition:
      "Математические модели, описывающие системы, которые меняются скачкообразно (в конкретные моменты времени) и имеют строго предсказуемое поведение.",
    relation:
      "Основа цифровой логики и компьютерных алгоритмов: состояние системы в момент T+1 полностью определяется состоянием в момент T.",
...
<span class="function">transition</span>(machine, <span class="string">'START'</span>); <span class="comment">// Всегда перейдет в 'RUNNING'</span>`,
  },
  {
    title: "React.js",
    definition:
      "JavaScript-библиотека для создания пользовательских интерфейсов с использованием компонентного подхода и декларативного управления состоянием.",
    relation:
      "Эффективно обновляет DOM через механизм Virtual DOM, позволяя разработчику описывать UI как функцию от состояния.",
...
<span class="keyword">const</span> <span class="function">App</span> = () => <span class="keyword">&lt;div&gt;</span>Hello, World!<span class="keyword">&lt;/div&gt;</span>;`,
  },
  {
    title: "Vercel",
    definition:
      "Облачная платформа для деплоя и хостинга фронтенд-приложений (особенно Next.js), ориентированная на высокую производительность и CI/CD.",
    relation:
      "Автоматизирует процесс доставки кода от Git-репозитория до глобальной сети доставки контента (Edge Network).",
...
{ <span class="string">"version"</span>: <span class="number">2</span>, <span class="string">"framework"</span>: <span class="string">"nextjs"</span> }`,
  },
  {
    title: "JavaScript",
    definition:
      "Мультипарадигменный язык программирования, являющийся основным инструментом для создания интерактивности в веб-браузерах и на сервере (Node.js).",
    relation:
      "Связующее звено между пользователем и логикой приложения, поддерживающее асинхронность и событийную модель.",
...
<span class="keyword">const</span> <span class="variable">data</span> = <span class="keyword">await</span> <span class="function">fetch</span>(<span class="string">'/api/data'</span>).<span class="function">json</span>();`,
  },
  {
    ...
    definition:
      "Статистическая величина, оценивающая изменчивость выборочного среднего. Показывает, насколько далеко выборочное среднее может находиться от истинного среднего популяции.",
    relation:
      "Ключевой показатель точности: чем меньше стандартная ошибка, тем надежнее наши данные.",
...
<span class="keyword">const</span> <span class="variable">standardError</span> = <span class="function">stdDev</span>(sample) / Math.<span class="function">sqrt</span>(sample.length);`,
  },
  {
    ...
    definition:
      "Относительная мера разброса данных, выраженная в процентах. Рассчитывается как отношение стандартного отклонения к среднему арифметическому.",
    relation:
      "Позволяет сравнивать изменчивость данных с разными единицами измерения или масштабами.",
...
const modelInstructions = {
  matrix: {
    ...
    goalNote:
      "Каждый клик влияет на крест из соседних клеток. Подумайте, какие ячейки включать, чтобы освещать сразу несколько элементов.",
  ...
  rings: {
    ...
    goalNote:
      "Сосредоточьтесь на каждом кольце как на уровне. Один поворот меняет не только текущее, но и соседние кольца.",
  ...
  graph: {
    ...
    goalNote:
      "Не выбирайте только самые дешевые ребра: ориентируйтесь на баланс стоимости и риска.",
...
function initResizableSidebars() {
  ...
  try {
    savedLeftWidth =
      localStorage.getItem("QadamAlgo_sidebar_left_width") || "220px";
    savedRightWidth =
      localStorage.getItem("QadamAlgo_sidebar_right_width") || "280px";
    leftHidden =
      localStorage.getItem("QadamAlgo_sidebar_left_hidden") === "true";
    rightHidden =
      localStorage.getItem("QadamAlgo_sidebar_right_hidden") === "true";
  } catch (e) {
  ...
  workspaceLayout.style.setProperty(
    "--sidebar-left-width",
    leftHidden ? "0px" : currentLeftWidth,
  );
  workspaceLayout.style.setProperty(
    "--sidebar-right-width",
    rightHidden ? "0px" : currentRightWidth,
  );
  ...
  const existingOpenRight = workspaceLayout.querySelector(
    ".sidebar-open-right",
  );
  ...
  leftOpenBtn.innerHTML =
    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="16" height="16"><polyline points="9 6 15 12 9 18"></polyline></svg>';
  leftOpenBtn.style.display = leftHidden ? "flex" : "none";

  const rightOpenBtn = document.createElement("button");
  rightOpenBtn.className =
    "btn btn-icon sidebar-open-toggle sidebar-open-right";
  rightOpenBtn.title = "Показать правую панель";
  rightOpenBtn.innerHTML =
    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="16" height="16"><polyline points="15 6 9 12 15 18"></polyline></svg>';
  ...
  const updateSidebarButtons = () => {
    leftOpenBtn.style.display = sidebarLeft.classList.contains("hidden")
      ? "flex"
      : "none";
    rightOpenBtn.style.display = sidebarRight.classList.contains("hidden")
      ? "flex"
      : "none";
  };
  ...
  toggleLeftBtn.addEventListener("click", () => {
    ...
    workspaceLayout.style.setProperty(
      "--sidebar-left-width",
      isHidden ? "0px" : currentLeftWidth,
    );
    toggleLeftBtn.title = isHidden ? "Показать панель" : "Скрыть панель";
    updateSidebarButtons();
    try {
      localStorage.setItem("QadamAlgo_sidebar_left_hidden", isHidden);
    } catch (e) {
  ...
  leftOpenBtn.addEventListener("click", () => {
    ...
    updateSidebarButtons();
    try {
      localStorage.setItem("QadamAlgo_sidebar_left_hidden", false);
    } catch (e) {
      console.warn("[SIDEBAR] Failed to save left sidebar state:", e.message);
    }
  });
  ...
  toggleRightBtn.addEventListener("click", () => {
    ...
    workspaceLayout.style.setProperty(
      "--sidebar-right-width",
      isHidden ? "0px" : currentRightWidth,
    );
    ...
    try {
      localStorage.setItem("QadamAlgo_sidebar_right_hidden", isHidden);
    } catch (e) {
  ...
  rightOpenBtn.addEventListener("click", () => {
    ...
    setTimeout(() => updateProgressChart(), 300);
    try {
      localStorage.setItem("QadamAlgo_sidebar_right_hidden", false);
    } catch (e) {
      console.warn("[SIDEBAR] Failed to save right sidebar state:", e.message);
    }
  });
  ...
  sidebarMouseMoveHandler = (e) => {
    ...
    if (resizeType === "left") {
      ...
      try {
        localStorage.setItem("QadamAlgo_sidebar_left_width", currentLeftWidth);
      } catch (e) {
...
function initNavigation() {
  ...
  const landingServiceCards = document.querySelectorAll(
    "#landing .service-card[data-service]",
  );
...
function initDashboard() {
  ...
  difficultyButtons.forEach((button) => {
    button.addEventListener("click", (e) => {
      ...
      const model = button.closest(".service-card").querySelector(".btn-card")
        .dataset.model;
  ...
  const difficultyGroups = document.querySelectorAll(
    "[data-model] .difficulty-group",
  );
  difficultyGroups.forEach((group) => {
    const buttons = group.querySelectorAll("button");
    const model = group.closest(".service-card").querySelector(".btn-card")
      .dataset.model;
...
function startModel(modelType, restore = false, showModal = false) {
  ...
  logTerminal(
    `[SYSTEM]: Выбрана сложность: ${getDifficultyLabel(difficulty)}`,
    "info",
  );
  ...
  const goalNote = modelInfo.goalNote
    ? `<div class="goal-explain">${modelInfo.goalNote}</div>`
    : "";
...
function initMatrixGame(container, difficulty = "medium") {
  ...
  const maxCellByWidth = Math.floor(
    (containerWidth - paddingAllowance - gap * (size - 1)) / size,
  );
  let cellSize =
    window.innerWidth <= 480 ? 60 : window.innerWidth <= 768 ? 65 : 85;
...
function restoreMatrixGame(container, save) {
  ...
  container.appendChild(grid);
  container.matrixData = { matrix, size };
  state.moveHistory = (save.moveHistory || []).map((history) =>
...
function buildMatrixPuzzle(size, difficulty) {
  ...
  console.warn(
    "[MATRIX] Could not find puzzle within thresholds, using relaxed constraints",
  );
...
function generateRandomMatrix(size, difficulty) {
  ...
  const [minPresses, maxPresses] =
    pressRanges[difficulty] || pressRanges.medium;
  const presses =
    minPresses + Math.floor(Math.random() * (maxPresses - minPresses + 1));
...
function getMinimalMatrixMoves(matrix) {
  ...
  for (let mask = 0; mask < 1 << size; mask++) {
    ...
    if (state[size - 1].every((v) => v === target)) {
      const count = press.reduce(
        (sum, row) => sum + row.reduce((s, value) => s + value, 0),
        0,
      );
      if (count < best) {
        best = count;
...
function initRingsGame(container, difficulty = "medium") {
  ...
  ringsContainer.style.setProperty(
    "--rings-top-marker-offset",
    `${ringsLayout.topMarkerOffset}px`,
  );
  ringsContainer.style.setProperty(
    "--rings-arrow-half",
    `${ringsLayout.arrowHalf}px`,
  );
  ringsContainer.style.setProperty(
    "--rings-arrow-height",
    `${ringsLayout.arrowHeight}px`,
  );
...
function buildRingsPuzzle(rings, difficulty) {
  ...
  for (let attempt = 0; attempt < maxTries; attempt++) {
    ...
    const minDistanceByDifficulty =
      difficulty === "easy" ? 4 : difficulty === "medium" ? 8 : 12;
    if (
      !isSolved &&
      misalignedCount >=
        Math.min(rings.length, difficulty === "hard" ? 4 : 3) &&
      totalDistance >= minDistanceByDifficulty
    ) {
      return;
    }
  }
...
function applyRingsMove(rings, ringIndex, direction, difficulty = "medium") {
  ...
  const applySteps = (ring, deltaSteps) => {
    ring.step =
      (((ring.step + deltaSteps) % ring.sectors) + ring.sectors) % ring.sectors;
  };
...
function createRingElement(ringData, index, allRings, container) {
  ...
  ring.addEventListener("click", (e) => {
    ...
    logTerminal(
      `[EVENT]: ${ringData.name} кольцо повернуто на ${direction > 0 ? "+" : "-"}1 шаг`,
      "event",
    );

    // Check win (exact discrete alignment)
    const aligned = allRings.every(
      (r) => ((r.step % r.sectors) + r.sectors) % r.sectors === 0,
    );
...
function initGraphGame(container, difficulty = "medium") {
  ...
  const graphDimensions = getEffectiveDimensions(
    graphBounds.width,
    graphBounds.height,
  );
  ...
  let { nodes, edges, optimalPath } = generateValidatedGraph(
    difficulty,
    graphWidth,
    graphHeight,
    minMoves,
  ...
  nodes.forEach((node) => {
    ...
    nodeEl.addEventListener("mouseenter", () => {
      const edge =
        adjMap[graphState.currentNode] &&
        adjMap[graphState.currentNode][node.id];
      if (edge) {
        const newCost = graphState.totalCost + edge.cost;
        const newRisk = graphState.totalRisk + edge.risk;
        const canMove =
          newRisk <= maxRisk && !graphState.path.includes(node.id);
...
function generateRandomGraph(
  ...
  console.log(
    "[GENERATE] Starting with difficulty:",
    difficulty,
    "containerWidth:",
    containerWidth,
    "containerHeight:",
  ...
  const usableHeight = Math.max(
    180,
    effectiveHeight - paddingTop - reservedBottom - nodeSize,
  );

  const difficultyExtra =
    difficulty === "easy" ? 1 : difficulty === "medium" ? 3 : 8;
  const difficultyPath =
    difficulty === "easy" ? 2 : difficulty === "medium" ? 4 : 7;
  const baseNodeCount =
    6 + Math.floor(difficultyExtra * responsiveConfig.multiplier);
  const randomExtra = isMobile
    ? 2 + Math.floor(Math.random() * 3)
    : 1 + Math.floor(Math.random() * 4);
  const pathLength = Math.max(
    minMoves,
    Math.ceil(difficultyPath * responsiveConfig.multiplier),
  );
  const mainPathCount = pathLength + 1;
  const desiredNodeCount = Math.max(
    baseNodeCount + randomExtra,
    mainPathCount + 1,
  );
  const maxNodesByArea = Math.max(
    mainPathCount,
    Math.floor(
      (usableWidth * usableHeight) / Math.max(1, nodeSize * nodeSize * 2.2),
    ),
  );
  const nodeCount = Math.max(
    mainPathCount,
    Math.min(desiredNodeCount, maxNodesByArea),
  );
  ...
  const waveAmplitude = Math.max(
    25,
    Math.min(usableHeight * 0.28, difficulty === "hard" ? 130 : 95),
  );
  for (let stage = 0; stage < mainPathCount; stage++) {
    ...
    const x =
      ...
      (Math.random() - 0.5) * (difficulty === "hard" ? 34 : 26);
    const waveOffset =
      Math.sin(
        progress * Math.PI * (difficulty === "hard" ? 2.5 : 2) + wavePhase,
      ) * waveAmplitude;
    ...
    positions.push({
      x: Math.max(
        paddingX,
        Math.min(
          effectiveWidth - paddingX - nodeSize,
          x + (Math.random() - 0.5) * 20,
        ),
      ),
      y: Math.max(
        reservedTop,
        Math.min(effectiveHeight - reservedBottom - nodeSize, y),
      ), // Respect reservedTop
      stage,
    });
  }
  ...
  for (let i = positions.length; i < nodeCount; i++) {
    ...
    do {
      const stage =
        1 + Math.floor(Math.random() * Math.max(1, mainPathCount - 2));
      x =
        paddingX +
        (usableWidth * stage) / (mainPathCount - 1) +
        (Math.random() - 0.5) * 140;
  ...
  for (let pass = 0; pass < 20; pass++) {
    // Increased to 20 passes
    for (let i = 0; i < positions.length; i++) {
      for (let j = i + 1; j < positions.length; j++) {
        const dx = positions[i].x - positions[j].x;
        const dy = positions[i].y - positions[j].y;
        const dist = Math.sqrt(dx * dx + dy * dy);
        if (dist < nodeSize * 1.1) {
          ...
          positions[j].x = Math.max(
            paddingX,
            Math.min(effectiveWidth - paddingX - nodeSize, positions[j].x),
          );
          positions[j].y = Math.max(
            reservedTop,
            Math.min(
              effectiveHeight - reservedBottom - nodeSize,
              positions[j].y,
            ),
          ); // Respect reservedTop
        }
  ...
  let remainingRiskBudget = Math.max(
    2,
    Math.floor(
      (difficulty === "easy" ? 0.75 : difficulty === "medium" ? 0.82 : 0.9) *
    ...
    for (let i = 0; i < stageNodes.length; i++) {
      for (let j = i + 1; j < stageNodes.length; j++) {
        ...
        if (!usedEdges.has(`${from}-${to}`)) {
          const cost =
            2 + Math.floor(Math.random() * (difficulty === "hard" ? 8 : 4));
          const risk =
            1 + Math.floor(Math.random() * (difficulty === "hard" ? 5 : 2));
          edges.push({ from, to, cost, risk });
          usedEdges.add(`${from}-${to}`);
          edges.push({ from: to, to: from, cost, risk });
          usedEdges.add(`${to}-${from}`);
  ...
  let extraAdded = 0;
  let attempts = 0;
  while (extraAdded < maxExtra && attempts < 1000) {
    // Increased attempts
    ...
    if (
      usedEdges.has(`${fromNode.id}-${toNode.id}`) ||
      usedEdges.has(`${toNode.id}-${fromNode.id}`)
    ) {
      attempts++;
      continue;
    }
    if (
      (fromNode.id === "S" && toNode.id === "F") ||
      (fromNode.id === "F" && toNode.id === "S")
    ) {
      attempts++;
      continue;
    }
    ...
    if (
      difficulty === "hard" &&
      toNode.id === "F" &&
      (fromNode.stage || 0) !== mainPathCount - 2
    ) {
      attempts++;
      continue;
    }
    ...
    const cost =
      1 + Math.floor(Math.random() * (difficulty === "hard" ? 15 : 6)); // Increased range
    const risk =
      1 + Math.floor(Math.random() * (difficulty === "hard" ? 10 : 4)); // Increased risk range
...
function generateValidatedGraph(
  difficulty,
  graphWidth,
  graphHeight,
  minMoves,
  ...
  for (let attempt = 0; attempt < 40; attempt++) {
    const candidate = generateRandomGraph(
      difficulty,
      graphWidth,
      graphHeight,
      minMoves,
    ...
    const minHops = findMinimumHopsWithinRisk(
      candidate.nodes,
      candidate.edges,
      maxRisk,
    );
    if (optimalPath.cost === Infinity || minHops === Infinity) continue;
    const isComplexEnough = isGraphComplexEnough(
      candidate.nodes,
      candidate.edges,
      maxRisk,
      optimalPath.path,
    ...
    if (
      ...
      (minHops === bestCandidate.minHops &&
        isComplexEnough &&
        !bestCandidate.isComplexEnough)
    ) {
      bestCandidate = { ...candidate, optimalPath, minHops, isComplexEnough };
    }
  }

  const minFallbackHops =
    difficulty === "hard"
      ? Math.max(5, minMoves - 1)
      : Math.max(3, minMoves - 2);
  if (bestCandidate && bestCandidate.minHops >= minFallbackHops) {
    console.warn(
      "[GRAPH] Using best fallback candidate with min hops:",
      bestCandidate.minHops,
    );
    return {
      nodes: bestCandidate.nodes,
      edges: bestCandidate.edges,
      optimalPath: bestCandidate.optimalPath,
    };
  }
  ...
  for (let attempt = 0; attempt < 20; attempt++) {
    ...
    if (optimalPath.cost !== Infinity && minHops !== Infinity) {
      console.warn(
        "[GRAPH] Using emergency fallback candidate with min hops:",
        minHops,
      );
      return { nodes: candidate.nodes, edges: candidate.edges, optimalPath };
    }
...
function isGraphComplexEnough(nodes, edges, maxRisk, optimalPath, difficulty) {
  ...
  const startChoices = (adj.S || []).filter(
    (edge) => edge.risk <= maxRisk,
  ).length;
  const minStartChoices =
  ...
  for (let i = 0; i < Math.min(checkDepth, optimalPath.length - 1); i++) {
    ...
    const alternatives = (adj[nodeId] || []).filter(
      (edge) =>
        edge.risk <= maxRisk && edge.to !== nextOptimal && edge.to !== "S",
    );
    if (alternatives.length > 0) branchPoints++;
  }
  const minBranchPoints =
    difficulty === "hard" ? 3 : difficulty === "medium" ? 2 : 1;
...
function handleGraphClick(
  ...
  if (!nodeId || !nodes || !adjMap) {
    console.error("[GRAPH] Invalid graph state:", {
      nodeId,
      nodesExists: !!nodes,
      adjMapExists: !!adjMap,
    });
  ...
  const edge =
    adjMap[graphState.currentNode] && adjMap[graphState.currentNode][nodeId];
  ...
  // Update info
  document.getElementById("graphCost").textContent =
    graphState.totalCost + " ₸";
  ...
  logTerminal(
    `[EVENT]: Перешли к ${nodes.find((n) => n.id === nodeId).label}. Стоимость: +${edge.cost} ₸, Риск: +${edge.risk}`,
    "event",
  );
...
function handleGraphWin(graphState, optimalPath) {
  ...
  if (costDiff === 0) {
    ...
    showToast(
      `Отлично! Ваш путь на ${costDiff} ₸ дороже оптимального`,
      "success",
    );
    logTerminal(
      `[SUCCESS]: Хороший путь! На ${costDiff} ₸ дороже оптимального.`,
      "success",
    );
    ...
    logTerminal(
      `[INFO]: Путь на ${costDiff} ₸ дороже оптимального.`,
      "warning",
    );
  ...
  logTerminal(
    `[STATS]: Ваша стоимость: ${graphState.totalCost} ₸, Оптимальная: ${optimalPath.cost} ₸`,
    "event",
  );
...
function handleWin(modelType) {
  ...
  if (state.isGameWon) {
    console.warn(
      "[GAME] Win handler already triggered, ignoring duplicate call",
    );
    return;
  }
  ...
  if (
    !state.solvedModels[modelType].maxDifficulty ||
    difficultyLevels[difficulty] >
      difficultyLevels[state.solvedModels[modelType].maxDifficulty]
  ) {
...
function computeSessionDataFromHistory(modelHistory) {
  ...
  const perAttemptScores = attempts.map((a) => {
    ...
    const model = ["matrix", "rings", "graph"].includes(a.modelType)
      ? a.modelType
      : "matrix";

    const baseT =
      (timeBaselineSec[model] && timeBaselineSec[model][difficulty]) ||
      timeBaselineSec[model].medium;
    const baseM =
      (moveBaseline[model] && moveBaseline[model][difficulty]) ||
      moveBaseline[model].medium;

    const speed = clamp(Math.round((baseT / seconds) * 100), 10, 100);
    const accuracy =
      moves === 0 ? 100 : clamp(Math.round((baseM / moves) * 100), 5, 100);
    ...
    const logic = clamp(
      Math.round((speed + accuracy) / 2) - movePenalty,
      5,
      100,
    );
  ...
  const avg = (key) =>
    Math.round(
      perAttemptScores.reduce((s, x) => s + x[key], 0) /
        perAttemptScores.length,
    );
...
function showCompletionModal(modelType, stats) {
  ...
  const descriptors = {
    matrix: {
      ...
      whyCool:
        "Это похоже на массовое применение правил к данным: одно действие влияет на группу зависимых элементов, и важно учитывать цепные эффекты.",
      serviceHow:
        "В сервисах наподобие Google Photos похожая логика нужна при пакетной обработке изображений: фильтры, метки и состояния применяются последовательно, а система должна сохранять согласованность итогового состояния.",
    },
    rings: {
      model: "Вложенные циклы",
      service: "Spotify / YouTube",
      action:
        "вы выровняли взаимозависимые кольца, где каждое вращение влияет на соседние уровни",
      whyCool:
        "Это тренирует мышление о каскадных эффектах: локальное изменение может сдвигать всю систему.",
      serviceHow:
        "В стриминговых сервисах похожий принцип возникает в пайплайнах рекомендаций и ранжирования: изменение одного фактора может перестраивать итоговую выдачу и приоритеты контента.",
    },
    graph: {
      model: "Поиск пути в графе",
      service: "Kaspi.kz Antifraud",
      action:
        "вы построили маршрут с балансом стоимости и риска в ограниченной сети",
      whyCool:
        "Это реальная задача принятия решений под ограничениями, где нужен не просто короткий путь, а оптимальный путь с учетом риска.",
      serviceHow:
        "В antifraud-системах Kaspi-подобного класса транзакции анализируются как граф связей: система ищет безопасные маршруты и паттерны, минимизируя риск при сохранении эффективности операций.",
    },
  };
...
function initWorkspace() {
  ...
  document.getElementById("solveHint").addEventListener("click", () => {
    const hints = {
      matrix:
        "Подсказка: Ячейки в углах влияют на меньшее количество соседей. Попробуйте работать от краев к центру.",
      rings:
        "Подсказка: Начните с внутреннего кольца, так как внешние влияют на внутренние.",
      graph:
        "Подсказка: Иногда путь с большей стоимостью но меньшим риском лучше, если он открывает выгодные маршруты.",
    };
  ...
  document
    .getElementById("completionModalOverlay")
...
function undoGraphMove(previousState) {
  ...
  graphData.graphState.visitedEdges = Array.isArray(previousState.visitedEdges)
    ? [...previousState.visitedEdges]
    : [];
  ...
  nodeEls.forEach((el) => {
    el.classList.remove("current", "visited");
    if (
      previousState.path.includes(el.dataset.id) &&
      el.dataset.id !== previousState.currentNode
    ) {
      el.classList.add("visited");
    }
  ...
  edgeEls.forEach((el) => {
    ...
    const isActive = previousState.visitedEdges.some(
...
function updateStatistics() {
  ...
  document.getElementById("avgAccuracy").textContent =
    state.sessionData.accuracy + "%";
  document.getElementById("efficiency").textContent =
    Math.round(
      (state.sessionData.speed +
        state.sessionData.accuracy +
        state.sessionData.logic +
        state.sessionData.algorithmic) /
        4,
    ) + "%";
...
Сгенерировано QadamAlgo
    ...
    a.href = url;
    a.download = "QadamAlgo-report.txt";
    a.click();
...
function saveToStorage() {
  ...
  try {
    localStorage.setItem("QadamAlgo_state", JSON.stringify(data));
  } catch (e) {
    if (e.name === "QuotaExceededError") {
...
function loadFromStorageWithFallback() {
  try {
    const saved = localStorage.getItem("QadamAlgo_state");
    if (saved) {
    ...
    if (e instanceof SyntaxError) {
      ...
      try {
        localStorage.removeItem("QadamAlgo_state");
      } catch (clearError) {
        console.error(
...
function loadFromStorage() {
  ...
  if (data) {
    ...
    if (
      data.currentPage === "workspace" &&
      data.currentModel === "matrix" &&
      data.matrixSave
    ) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!expCards.length || !modalOverlay) return;

  expCards.forEach((card) => {
    card.addEventListener("click", () => {
      document.getElementById("studentModalTitle").textContent =
        card.dataset.name;
      document.getElementById("studentGroupHighlight").innerHTML =
        `Группа: <strong>${card.dataset.group}</strong>`;
      document.getElementById("studentPre").textContent = card.dataset.pre;
      document.getElementById("studentPost").textContent = card.dataset.post;
      document.getElementById("studentAbs").textContent = card.dataset.abs;
      document.getElementById("studentRel").textContent = card.dataset.rel;

      document.getElementById("studentPdfViewer").src =
        card.dataset.pdf + "#toolbar=0&navpanes=0&view=FitH";
...
function runFormularCalculations() {
  const table = document.getElementById("expDataTable");
  if (!table) return;

  const rows = table.querySelectorAll("tbody tr");
  const data = { alpha: { pre: [], post: [] }, beta: { pre: [], post: [] } };

  rows.forEach((row) => {
    const cells = row.cells;
    const pre = parseFloat(cells[2].textContent);
    const post = parseFloat(cells[3].textContent);
    const group = cells[1].textContent.includes("Бета") ? "beta" : "alpha";

    const abs = post - pre;
    const rel = pre !== 0 ? (abs / pre) * 100 : 0;
    cells[4].textContent = (abs >= 0 ? "+" : "") + abs;
    cells[5].textContent = rel.toFixed(1) + "%";

    data[group].pre.push(pre);
    data[group].post.push(post);
  });

  const calcStats = (arrPre, arrPost, prefix) => {
    const avg = (a) => a.reduce((sum, v) => sum + v, 0) / a.length;
    const median = (a) => {
    ...
    const mode = (a) => {
      ...
      return Object.keys(counts).reduce((x, y) =>
        counts[x] > counts[y] ? x : y,
      );
    };
    const std = (a) => {
      const m = avg(a);
      return Math.sqrt(
        a.reduce((s, v) => s + Math.pow(v - m, 2), 0) / a.length,
      );
    };

    const meanPre = avg(arrPre);
    const meanPost = avg(arrPost);
    const growth = ((meanPost - meanPre) / meanPre) * 100;

    document.getElementById(`avgPre${prefix}`).textContent = meanPre.toFixed(1);
    document.getElementById(`avgPost${prefix}`).textContent =
      meanPost.toFixed(1);
    document.getElementById(`growth${prefix}`).textContent =
      Math.round(growth) + "%";
    document.getElementById(`median${prefix}`).textContent = median(arrPost);
    document.getElementById(`mode${prefix}`).textContent = mode(arrPost);
    document.getElementById(`std${prefix}`).textContent =
      std(arrPost).toFixed(2);
  };

  calcStats(data.alpha.pre, data.alpha.post, "Alpha");
  calcStats(data.beta.pre, data.beta.post, "Beta");
}

document.addEventListener("DOMContentLoaded", runFormularCalculations);

function makeTableMobileFriendly() {
  const summaryTable = document.querySelector(".exp-container table");
  if (!summaryTable) return;

  const headers = Array.from(summaryTable.querySelectorAll("th")).map(
    (th) => th.textContent,
  );
...
document.addEventListener("DOMContentLoaded", () => {
  runFormularCalculations();
  makeTableMobileFriendly();
});

function setupMobileLabels() {
  const resultsTable = document.getElementById("expDataTable");
  if (!resultsTable) return;

  const headers = Array.from(resultsTable.querySelectorAll("thead th")).map(
